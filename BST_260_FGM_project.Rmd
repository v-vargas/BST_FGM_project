...
Additionally, the methods that we planned to replicate included both women born in countries that practice FGM and their daughters. We were not able to find a usable data on these women's daughters and believe that the methods used in the paper might be incorrect. We settled on doing a more conservative and valid estimate of FGM by just including women born in the countries that practice FGM, and excluding daughters. We were able to find a variable in the ACS dataset that was "presence of daughter in the household", but does not specify the number of daughters. We believe this is the indicator the authors may have used that could have resulted in an incorrect estimate and have contacted the authors for furhter clarification. 
...
We also create a world map of FGM prevalence to visualize the prevalence of FGM in countries where it is practiced. This map though does not represent migration patterns, which further emphasizes the necessity of our analysis. 

### Final analysis:

To obtain the estimate the number of women living in the US who had experienced FGM, we created a new variable that was the multiplication of the country FGM estimate and the number of women born in that country, currently living in the US. When doing our analysis using the first method described, we calculated a prevalence of 370,055. The paper mentioned previously found an estimate of 545,000. It makes sense that our estimate is less than the estimate in the paper as we did not include daughters at risk in our estimate due to data constraints and concerns regarding assumptions the authors made. 
